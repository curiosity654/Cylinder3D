...
@register_dataset
class voxel_dataset_with_inst(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=255, return_test=False,
                 fixed_volume_space=False, max_volume_space=[50, 50, 1.5], min_volume_space=[-50, -50, -3]):
        'Initialization'
        self.point_cloud_dataset = in_dataset
    ...
    def __getitem__(self, index):
        ...
        data = self.point_cloud_dataset[index]
        if len(data) == 3:
            xyz, labels, inst_id = data
        elif len(data) == 4:
            xyz, labels, sig, inst_id = data
            if len(sig.shape) == 2: sig = np.squeeze(sig)
        else:
            raise Exception('Return invalid data tuple')
        ...
        if self.return_test:
            ...
            data_tuple += (grid_ind, labels, inst_id, return_fea,)
        return data_tuple

def collate_fn_BEV_with_inst(data):
    data2stack = np.stack([d[0] for d in data]).astype(np.float32)
    label2stack = np.stack([d[1] for d in data]).astype(np.int)
    grid_ind_stack = [d[2] for d in data]
    point_label = [d[3] for d in data]
    inst_label = [d[4] for d in data]
    xyz = [d[5] for d in data]
    return torch.from_numpy(data2stack), torch.from_numpy(label2stack), grid_ind_stack, point_label, inst_label, xyz
...
from dbsampler.dbsampler import DataBaseSampler
@register_dataset
class cylinder_dataset_dbsample(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=255, return_test=False,
                 fixed_volume_space=False, max_volume_space=[50, np.pi, 2], min_volume_space=[0, -np.pi, -4],
                 scale_aug=False,
                 transform_aug=False, trans_std=[0.1, 0.1, 0.1],
        ...
        info_path = "/root/code/Cylinder3D-dev/data/semkitti/database/semantickitti_gt_database.pkl"
        database_root = "/root/code/Cylinder3D-dev/data/semkitti/database"

        self.dbsampler = DataBaseSampler(info_path, database_root)

        self.sample_dict = {'car': 1, "bicycle": 1, "motorcycle": 1, 'person': 1, 'bicyclist': 1, 'motorcyclist': 0}
        self.cat2id = {'car': 1, 'bicycle': 2, 'motorcycle': 3, 'person': 6, 'bicyclist': 7, 'motorcyclist': 8}
        self.avoid_class = np.array([1,2,3,4,5,6,7,8,13,14,16,18,19])
    ...
    def __getitem__(self, index):
        ...
        if self.rotate_aug:
            # TODO dbsample only for training !!!
            xyz, labels = self.dbsampler.sample_class(xyz, labels, self.sample_dict, self.cat2id, self.avoid_class)
