...
def seesaw_ce_loss(cls_score,
    ...
    if p > 0:
        ...
        seesaw_weights = seesaw_weights * mitigation_factor.cuda()
    ...
    if q > 0:
        ...
        seesaw_weights = seesaw_weights * compensation_factor.cuda()

    cls_score = cls_score + (seesaw_weights.log() * (1 - onehot_labels))

    loss = F.cross_entropy(cls_score, labels, weight=None, reduction='none', ignore_index=0)

    if label_weights is not None:
        label_weights = label_weights.float()
    loss = weight_reduce_loss(
        loss, weight=label_weights, reduction=reduction, avg_factor=avg_factor)
    return loss, sample_weights, score_matrix, seesaw_weights
    ...
    def forward(self,
        ...
        for u_l in unique_labels:
            ...
            if u_l.item() != 0:
                ...
                self.cum_samples[u_l] += inds_.cpu().sum()
        ...
        loss_cls, mitigation_factor, compensation_factor, seesaw_weights = self.cls_criterion(
            cls_score, labels,
